...
`NewRedisGenerator` seeds IDs `0..maxWorkerID` on first use and stores `max_worker_id` beside the zset (same `{workerid:cluster:%s}` hash tag). A later process that joins the same cluster name with a different `WithWorkerBits` gets `ErrClusterConfigMismatch`. If the zset already exists without that meta key, the stored max is inferred as `ZCARD-1` (contiguous `0..N` pools).
